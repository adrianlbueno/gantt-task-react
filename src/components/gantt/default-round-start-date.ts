...
export const defaultRoundStartDate = (
  ...
  switch (viewMode) {
    ...
    case ViewMode.QuarterYear: {
      ...
      if (diff < 6) return addMonths(start, 3);
      if (diff < 9) return addMonths(start, 6);
      if (diff < 12) return addMonths(start, 9);
    ...
    case ViewMode.HalfYear: {
      const start = startOfYear(date);
      const diff = differenceInMonths(date, start);
      if (diff < 6) return start;
      if (diff < 12) return addMonths(start, 6);
      return addYears(start, 1);
    }
